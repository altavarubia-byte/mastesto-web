...
export default function PerfilPage() {
  ...
  const consultarMentor = async (e: React.FormEvent) => {
    ...
    const contextoTotal = `
      ...
      TAREAS ACTIVAS: ${tareasReales || "Sin tareas registradas en el frente"}.
      MISIÓN: Disciplina absoluta en +TESTO.
    `;
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-12 font-sans relative overflow-hidden">
      ...
      <div className="fixed bottom-10 right-10 z-50 flex flex-col items-end">
        {isOpen && (
          <div className="mb-6 w-80 md:w-96 bg-zinc-950 border-2 border-orange-600 rounded-[2.5rem] overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300 flex flex-col">
            ...
            <div ref={scrollRef} className="h-64 overflow-y-auto p-6 font-mono text-[10px] uppercase bg-black text-orange-500 space-y-4 border-b border-zinc-900">
              {chat.length === 0 && <p className="opacity-40 italic text-center text-[8px]">EL FORJADOR ESTÁ OBSERVANDO TU PROGRESO.</p>}
              {chat.map((msg, i) => (
